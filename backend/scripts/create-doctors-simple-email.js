...
// Doctor data by department with simple email prefixes
...
async function createDoctorsWithSimpleEmail() {
  console.log('👨‍⚕️ CREATING DOCTORS WITH SIMPLE EMAIL FORMAT');
  ...
  try {
    ...
    for (const [deptId, deptData] of Object.entries(doctorsByDepartment)) {
      ...
      for (let i = 0; i < deptData.doctors.length; i++) {
        ...
        // Create simple email format: prefix + number
        const email = `${deptData.emailPrefix}${i + 1}@hospital.com`;
        
        console.log(`   📝 Creating: ${doctorData.name} (${email})`);
      ...
      for (let i = 0; i < createdAuthUsers.length; i++) {
        ...
        const licenseNum = `VN-${deptData.emailPrefix.toUpperCase()}-${(1000 + totalCreated + i).toString()}`;
    ...
    console.log(`   cardio1@hospital.com / Doctor123!  (Tim mạch)`);
    console.log(`   ortho1@hospital.com / Doctor123!   (Chấn thương)`);
    console.log(`   neuro1@hospital.com / Doctor123!   (Thần kinh)`);
    console.log(`   obgyn1@hospital.com / Doctor123!   (Phụ sản)`);
    console.log(`   internal1@hospital.com / Doctor123! (Nội tổng hợp)`);
    console.log(`   surgery1@hospital.com / Doctor123!  (Ngoại tổng hợp)`);
...
async function main() {
  await createDoctorsWithSimpleEmail();
}
