-- =====================================================
-- FIX TRIGGER DEPENDENCY
-- Hospital Management System - Drop trigger first, then function
-- =====================================================

-- Step 1: Drop trigger first (this removes the dependency)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Step 2: Now we can drop the function
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Step 3: Drop all existing versions of create_user_profile function
...
-- Step 4: Create sequence for doctor IDs if not exists
CREATE SEQUENCE IF NOT EXISTS doctor_id_seq START 1;

-- Step 5: Create the enhanced trigger function
...
-- Step 7: Create the enhanced RPC function with unique name
...
-- Create new RLS policies that allow trigger to work
...
BEGIN
    ...
    RETURN json_build_object(
        ...
        'enhanced_rpc_exists', EXISTS(
            SELECT 1 FROM information_schema.routines 
            WHERE routine_name = 'create_user_profile_enhanced'
        ),
        'timestamp', NOW()
    );
END;
...
SELECT 'SETUP COMPLETED' as status, public.test_trigger_status() as result;
