 ...
 * @version 2.0.0
 * @compliance Clean Architecture, Repository Pattern, Schema-per-Service
 */
 ...
 * Implements billing repository using Supabase
 */
  ...
  async save(billing: BillingAggregate): Promise<void> {
    try {
      ...
      if (billing.items.length > 0) {
        const itemRecords = billing.items.map((item) =>
          this.mapItemToRecord(item, billing.id.value)
        );
        ...
        if (itemsError) {
          throw new Error(
            `Failed to save billing items: ${itemsError.message}`
          );
        }
      }
