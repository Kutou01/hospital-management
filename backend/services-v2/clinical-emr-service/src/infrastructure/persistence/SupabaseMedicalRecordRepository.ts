...
{
  constructor(private readonly supabaseClient: OptimizedSupabaseClient) {}
  ...
  async save(medicalRecord: MedicalRecordAggregate): Promise<void> {
    try {
      ...
      if (existingRecord) {
        throw new MedicalRecordAlreadyExistsError(medicalRecord.recordId.value);
      }

      // Map aggregate to database format
      const dbRecord = this.mapAggregateToDatabase(medicalRecord);

      const { error } = await client.from("medical_records").insert(dbRecord);

      if (error) {
        throw new MedicalRecordRepositoryError(
          `Failed to save medical record: ${error.message}`,
          "SAVE_FAILED"
        );
      }
    } catch (error) {
      ...
      throw new MedicalRecordRepositoryError(
        `Repository error during save: ${error instanceof Error ? error.message : "Unknown error"}`,
        "REPOSITORY_ERROR"
      );
    }
  }
  ...
  async findById(recordId: RecordId): Promise<MedicalRecordAggregate | null> {
    try {
      ...
      if (error) {
        ...
        throw new MedicalRecordRepositoryError(
          `Failed to find medical record: ${error.message}`,
          "FIND_FAILED"
        );
      }
      ...
      throw new MedicalRecordRepositoryError(
        `Repository error during findById: ${error instanceof Error ? error.message : "Unknown error"}`,
        "REPOSITORY_ERROR"
      );
    }
  }
