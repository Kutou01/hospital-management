...
export class NotificationService {
  ...
  constructor() {
    ...
    logger.info('🔔 Notification Service initialized', { enabled: this.isEnabled });
  }
  ...
  async sendAppointmentCreatedNotification(appointment: Appointment): Promise<void> {
    if (!this.isEnabled) {
      logger.info('📵 Notifications disabled - skipping appointment created notification');
      return;
    }
  ...
  async sendAppointmentUpdatedNotification(
    ...
    if (!this.isEnabled) {
      logger.info('📵 Notifications disabled - skipping appointment updated notification');
      return;
    }
    ...
    try {
      ...
      logger.error('❌ Error sending appointment updated notifications:', error);
    }
  }
  ...
  async sendAppointmentCancelledNotification(
    ...
    if (!this.isEnabled) {
      logger.info('📵 Notifications disabled - skipping appointment cancelled notification');
      return;
    }
    ...
    try {
      ...
      logger.error('❌ Error sending appointment cancelled notifications:', error);
    }
  }
