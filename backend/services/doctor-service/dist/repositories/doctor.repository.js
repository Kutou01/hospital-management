...
class DoctorRepository {
    ...
    async findByProfileId(profileId) {
        try {
            ...
            if (error) {
                logger_1.default.error('Database RPC error in findByProfileId:', error);
                ...
                if (doctorError || !doctorData) {
                    logger_1.default.error('Doctor not found:', doctorError);
                    return null;
                }
                const { data: profileData, error: profileError } = await this.supabase
                    ...
                    .single();
                if (profileError) {
                    logger_1.default.error('Profile not found:', profileError);
                    return null;
                }
                const combinedData = {
                    ...doctorData,
