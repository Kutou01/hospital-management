...
class DoctorRealtimeService {
    ...
    async handleProfileChange(payload) {
        try {
            ...
            logger_1.default.error('❌ Error handling doctor profile change:', error);
        }
    }
    async handleShiftChange(payload) {
        try {
            ...
            logger_1.default.error('❌ Error handling doctor shift change:', error);
        }
    }
    async handleExperienceChange(payload) {
        try {
            ...
            logger_1.default.error('❌ Error handling doctor experience change:', error);
        }
    }
    ...
    async publishToEventBus(event) {
        try {
            await this.eventBus.publish('doctor_changed', event, `doctor.${event.type.toLowerCase()}`);
            if (event.availability_updated) {
                await this.eventBus.publish('doctor_availability_updated', event, 'doctor.availability');
            }
            if (event.schedule_updated || event.shift_updated) {
                await this.eventBus.publish('doctor_schedule_updated', event, 'doctor.schedule');
            }
        }
        catch (error) {
    ...
    async handleNewDoctor(event) {
        logger_1.default.info('🆕 New doctor registered:', event.doctor_id);
    ...
    async handleScheduleUpdate(event) {
        logger_1.default.info('⏰ Schedule updated for doctor:', event.doctor_id);
    }
