...
export class AvailabilityService {
   ...
   */
  async getDoctorAvailability(query: AvailabilityQuery): Promise<DoctorAvailability | null> {
    try {
      const { doctor_id, date, duration = 30, include_breaks = false } = query;
      
      logger.info('🔄 Getting doctor availability', { doctor_id, date, duration });
   ...
   */
  async getAvailableTimeSlots(doctor_id: string, date: string, duration: number = 30): Promise<TimeSlot[]> {
    try {
      const availability = await this.getDoctorAvailability({ doctor_id, date, duration });
  ...
  async isTimeSlotAvailable(
    ...
    try {
      logger.info('🔄 Checking time slot availability', { doctor_id, date, start_time, end_time });
      ...
      if (!this.isWithinWorkingHours(start_time, end_time, schedule)) {
        logger.info('⚠️ Time outside working hours', { start_time, end_time, schedule });
        return false;
      }
      ...
      if (hasConflict) {
        logger.info('⚠️ Time slot conflicts with existing appointment', { doctor_id, date, start_time, end_time });
        return false;
      }

      logger.info('✅ Time slot is available', { doctor_id, date, start_time, end_time });
      return true;
    } catch (error) {
   ...
   */
  private async getDoctorSchedule(doctor_id: string, date: string): Promise<any> {
    try {
   ...
   */
  private async getDoctorAppointments(doctor_id: string, date: string): Promise<any[]> {
    try {
      const { data, error } = await supabaseAdmin
        .from('appointments')
        .select(`
  ...
  private async generateTimeSlots(
    ...
    for (let currentMinutes = startMinutes; currentMinutes < endMinutes; currentMinutes += duration) {
      ...
      const slot: TimeSlot = {
        ...
        max_appointments: 1,
        slot_type: isDuringBreak ? 'break' : 
                  conflictingAppointment ? 'booked' : 'available'
      };

      if (conflictingAppointment) {
        slot.appointment_id = conflictingAppointment.appointment_id;
        slot.patient_name = conflictingAppointment.patients?.profiles?.full_name;
   ...
   * Check if time is within working hours
   */
  private isWithinWorkingHours(start_time: string, end_time: string, schedule: any): boolean {
