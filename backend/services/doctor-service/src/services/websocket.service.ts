...
export class WebSocketManager {
  ...
  async initialize(httpServer?: HttpServer): Promise<void> {
    try {
      ...
      logger.info('✅ Doctor WebSocket Manager initialized successfully on HTTP server');
    } catch (error) {
  ...
  private handleAuthentication(socket: Socket, client: ConnectedClient, data: any): void {
    try {
      ...
      logger.error('❌ Doctor service authentication failed for client:', socket.id, error);
    }
  }
   ...
   * Subscribe to doctor updates
   */
  private subscribeToDoctorUpdates(socket: Socket, client: ConnectedClient, doctor_id: string): void {
  ...
  public broadcastToRoom(roomName: string, event: string, data: any): void {
    if (!this.io || !this.isInitialized) {
      logger.warn('⚠️ Doctor WebSocket not initialized - skipping room broadcast');
      return;
    }
  ...
  public sendToClient(clientId: string, event: string, data: any): void {
    if (!this.io || !this.isInitialized) {
      logger.warn('⚠️ Doctor WebSocket not initialized - skipping client message');
      return;
    }
