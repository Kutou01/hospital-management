...
export const appointmentResolvers = {
  Query: {
    ...
    async appointment(
      ...
      try {
        ...
        if (!identifier) {
          throw new Error(context.i18n.translate('appointment.errors.missing_identifier'));
        }

        logger.debug('Fetching appointment:', { identifier, requestId: context.requestId });

        const response = await context.restApi.getAppointment(identifier);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.fetch_failed'));
        }
    ...
    async appointments(
      _: any,
      { filters, limit = 20, offset = 0, sortBy = 'scheduledDateTime', sortOrder = 'ASC' }: any,
      context: GraphQLContext
    ) {
      try {
        logger.debug('Fetching appointments:', { filters, limit, offset, requestId: context.requestId });
    ...
    async todayAppointments(
      ...
      try {
        logger.debug('Fetching today appointments:', { doctorId, departmentId, status, requestId: context.requestId });
        ...
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.today_failed'));
        }
    ...
    async upcomingAppointments(
      ...
      try {
        logger.debug('Fetching upcoming appointments:', { doctorId, patientId, days, limit, requestId: context.requestId });
        ...
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.upcoming_failed'));
        }
    ...
    async availableSlots(
      _: any,
      { doctorId, date, duration = 30 }: { doctorId: string; date: string; duration: number },
      context: GraphQLContext
    ) {
      try {
        logger.debug('Fetching available slots:', { doctorId, date, duration, requestId: context.requestId });
    ...
    async appointmentStats(
      ...
      try {
        logger.debug('Fetching appointment stats:', { doctorId, patientId, departmentId, requestId: context.requestId });
        ...
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.stats_failed'));
        }
  ...
  Mutation: {
    ...
    async createAppointment(
      ...
      try {
        logger.debug('Creating appointment:', { input, requestId: context.requestId });

        const response = await context.restApi.createAppointment(input);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.create_failed'));
        }

        logger.info('Appointment created successfully:', { appointmentId: response.data.appointmentId });
        return response.data;
      } catch (error) {
    ...
    async updateAppointment(
      ...
      try {
        logger.debug('Updating appointment:', { id, input, requestId: context.requestId });

        const response = await context.restApi.updateAppointment(id, input);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.update_failed'));
        }
    ...
    async cancelAppointment(
      ...
      try {
        logger.debug('Cancelling appointment:', { id, reason, requestId: context.requestId });

        const response = await context.restApi.cancelAppointment(id, reason);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.cancel_failed'));
        }
    ...
    async confirmAppointment(
      ...
      try {
        logger.debug('Confirming appointment:', { id, requestId: context.requestId });

        const response = await context.restApi.confirmAppointment(id);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.confirm_failed'));
        }

        logger.info('Appointment confirmed successfully:', { appointmentId: id });
        return response.data;
      } catch (error) {
    ...
    async rescheduleAppointment(
      _: any,
      { id, newDate, newTime }: { id: string; newDate: string; newTime: string },
      context: GraphQLContext
    ) {
      try {
        logger.debug('Rescheduling appointment:', { id, newDate, newTime, requestId: context.requestId });

        const response = await context.restApi.rescheduleAppointment(id, { newDate, newTime });
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.reschedule_failed'));
        }

        logger.info('Appointment rescheduled successfully:', { appointmentId: id });
        return response.data;
      } catch (error) {
    ...
    async checkInAppointment(
      ...
      try {
        logger.debug('Checking in appointment:', { id, requestId: context.requestId });

        const response = await context.restApi.checkInAppointment(id);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.checkin_failed'));
        }
    ...
    async completeAppointment(
      ...
      try {
        logger.debug('Completing appointment:', { id, notes, requestId: context.requestId });

        const response = await context.restApi.completeAppointment(id, notes);
        
        if (!response.success) {
          throw new Error(response.error?.message || context.i18n.translate('appointment.errors.complete_failed'));
        }
  ...
  Appointment: {
    ...
    isToday(parent: any) {
      ...
      const today = new Date().toISOString().split('T')[0];
      const appointmentDate = new Date(parent.scheduledDate).toISOString().split('T')[0];
    ...
    canCancel(parent: any) {
      if (!parent.scheduledDateTime || parent.status === 'CANCELLED' || parent.status === 'COMPLETED') {
        return false;
      }
