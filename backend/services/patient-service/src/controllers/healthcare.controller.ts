...
export class PatientHealthcareController {
  ...
  validatePatientFHIR = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.info(`Validating patient FHIR compliance for patient: ${patient_id}`);
      ...
      logger.error('Patient Healthcare Controller - Validate patient FHIR failed:', error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to validate patient FHIR compliance',
  ...
  getPatientFHIR = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.error('Patient Healthcare Controller - Get patient FHIR failed:', error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to convert patient to FHIR format',
   ...
   */
  getPatientMedicalHistory = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id: patient_id } = req.params;
      ...
      if (!diagnosesResult.success) {
        res.status(400).json(diagnosesResult);
        return;
      }

      // Filter diagnoses based on query parameter
      let diagnoses = diagnosesResult.data || [];
      ...
      logger.error('Patient Healthcare Controller - Get medical history failed:', error);
      res.status(500).json({
        success: false,
   ...
   */
  getDiagnosesByCategory = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const filteredDiagnoses = (diagnosesResult.data || []).filter(diagnosis => {
      ...
      logger.error('Patient Healthcare Controller - Get diagnoses by category failed:', error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to get diagnoses by category',
   ...
   */
  getHealthcareTimeline = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id: patient_id } = req.params;
      ...
      if (start_date) {
        diagnoses = diagnoses.filter(d => d.diagnosis_date >= start_date);
      }
      if (end_date) {
        diagnoses = diagnoses.filter(d => d.diagnosis_date <= end_date);
      }
      ...
      logger.error('Patient Healthcare Controller - Get healthcare timeline failed:', error);
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to get healthcare timeline',
        code: 'TIMELINE_ERROR'
      });
    }
  };
  ...
  getHealthcareCompliance = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      logger.info(`Getting healthcare compliance status for patient: ${patient_id}`);
      ...
      const diagnosesResult = await this.healthcareService.getPatientDiagnoses(patient_id);
      const diagnosesCount = diagnosesResult.data?.length || 0;
      ...
      logger.error('Patient Healthcare Controller - Get healthcare compliance failed:', error);
      res.status(500).json({
        success: false,
  ...
  getHealthSummary = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const summary = {
        total_diagnoses: diagnoses.length,
        active_conditions: diagnoses.filter(d => d.status === 'active').length,
        chronic_conditions: diagnoses.filter(d => d.status === 'chronic').length,
        ...
        follow_up_required: diagnoses.filter(d => d.follow_up_required && d.status === 'active').length,
      ...
      logger.error('Patient Healthcare Controller - Get health summary failed:', error);
      res.status(500).json({
        success: false,
