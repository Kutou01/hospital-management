...
export function OAuthLoginButtons({ 
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github' | 'facebook' | 'apple') => {
    ...
    try {
      ...
      showToast("❌ Lỗi", `Không thể đăng nhập với ${provider}`, "error")
    } finally {
    ...
    <Card className={className}>
      {showTitle && (
        <CardHeader className="pb-4">
          <CardTitle className="text-center text-lg">Đăng nhập nhanh</CardTitle>
        </CardHeader>
      )}
      <CardContent className="space-y-4">
        {providers.map((provider) => (
          <Button
            key={provider.id}
            variant="outline"
            className={`w-full h-12 ${provider.bgColor} ${provider.textColor} border ${provider.borderColor} transition-colors`}
